...
__attribute__((aligned(4))) uint8_t IFTest_Buf[53] = {
    ...
    0xAA, 0xEE, 0xEE, 0xEE, 0xEE, 0xEE, 0xEE, 0xEE, 0xEE, 0xFE,       // 26
...
volatile uint8_t USBHS_DevCurInterface;

/* Endpoint Buffer */
__attribute__((aligned(4))) uint8_t USBHS_EP0_Buf[DEF_USBD_UEP0_SIZE];
__attribute__((aligned(4))) uint8_t USBHS_EP1_Rx_Buf[2][DEF_USB_EP1_HS_SIZE];
volatile uint8_t USBHS_EP1_Rx_Buf_PTR;
__attribute__((aligned(4))) uint8_t USBHS_EP1_Tx_Buf[DEF_USB_EP1_FD_HS_SIZE];
...
/* Audio_Control */
uint8_t SampleFreq[] = {SAMPLE_FREQ_4B(USBD_AUDIO_FREQ_384)};
const uint8_t SampleRangeFreq[] = {
    0x07,
    0x00,                                // wNumSubRanges
    SAMPLE_FREQ_4B(USBD_AUDIO_FREQ_384), // dMIN
    SAMPLE_FREQ_4B(USBD_AUDIO_FREQ_384), // dMAX
    SAMPLE_FREQ_4B(0x00),                // dRES
    SAMPLE_FREQ_4B(USBD_AUDIO_FREQ_192),
    SAMPLE_FREQ_4B(USBD_AUDIO_FREQ_192),
    SAMPLE_FREQ_4B(0x00),
    SAMPLE_FREQ_4B(USBD_AUDIO_FREQ_96),
    SAMPLE_FREQ_4B(USBD_AUDIO_FREQ_96),
    SAMPLE_FREQ_4B(0x00),
    SAMPLE_FREQ_4B(USBD_AUDIO_FREQ_48),
    SAMPLE_FREQ_4B(USBD_AUDIO_FREQ_48),
    SAMPLE_FREQ_4B(0x00),
    SAMPLE_FREQ_4B(USBD_AUDIO_FREQ_441),
    SAMPLE_FREQ_4B(USBD_AUDIO_FREQ_441),
    SAMPLE_FREQ_4B(0x00),
    SAMPLE_FREQ_4B(USBD_AUDIO_FREQ_882),
    SAMPLE_FREQ_4B(USBD_AUDIO_FREQ_882),
    SAMPLE_FREQ_4B(0x00),
    SAMPLE_FREQ_4B(USBD_AUDIO_FREQ_1764),
    SAMPLE_FREQ_4B(USBD_AUDIO_FREQ_1764),
    SAMPLE_FREQ_4B(0x00),
};
...
void USBHS_Device_Endp_Init(void)
{
    /*
     * Endpoint0: System.
     * Endpoint1: Audio streaming and feedback.
     */
    ...
    USBHSD->UEP1_RX_DMA = (uint32_t)(uint8_t *)USBHS_EP1_Rx_Buf[0];
    ...
    USBHSD->UEP1_RX_CTRL = USBHS_UEP_T_TOG_DATA0;
    ...
    if ((endp >= DEF_UEP1) && (endp <= DEF_UEP15))
    {
        ...
        if (endp_en & USBHSD_UEP_TX_EN(endp))
        {
            if ((USBHS_Endp_Busy[endp] & DEF_UEP_BUSY) == 0x00)
            {
                ...
                USBHSD_UEP_TXCTRL(endp) = (USBHSD_UEP_TXCTRL(endp) &=
    ...
    if (intflag & USBHS_UIF_TRANSFER)
    {
        ...
        case USBHS_UIS_TOKEN_IN:
            ...
            case USBHS_UIS_TOKEN_IN | DEF_UEP0:
                ...
                else if ((USBHS_SetupReqType & USB_REQ_TYP_MASK) == USB_REQ_TYP_CLASS)
                {
                    /* Non-standard request endpoint 0 Data upload */
                    if (USBHS_SetupReqType & AUDIO_REQ_GET_MSK)
                    { // GET
                        switch (USBHS_SetupReqCode)
                        {
        ...
        case USBHS_UIS_TOKEN_OUT:
            ...
            case USBHS_UIS_TOKEN_OUT | DEF_UEP0:
                ...
                if (intst & USBHS_UIS_TOG_OK)
                {
                    ...
                    else if ((USBHS_SetupReqType & USB_REQ_TYP_MASK) == USB_REQ_TYP_CLASS)
                    {
                        ...
                        if (!(USBHS_SetupReqType & AUDIO_REQ_GET_MSK))
                        {                                            // SET
                            if (USBHS_SetupReqCode == AUDIO_REQ_CUR) // CUR
                            {
                                if (USBHS_SetupReqIndex == (AUDIO_CLOCK_ID << 8) && USBHS_SetupReqValue == (CS_SAM_FREQ_CONTROL << 8))
                                { // Clock
                                    if (USBHS_SetupReqLen == 4)
                                    {
                                        freq = 0;
                                        freq |= USBHS_EP0_Buf[3];
                                        freq = (freq << 8) | USBHS_EP0_Buf[2];
                                        freq = (freq << 8) | USBHS_EP0_Buf[1];
                                        freq = (freq << 8) | USBHS_EP0_Buf[0];
                                        if (memcmp(SampleFreq, USBHS_EP0_Buf, sizeof(SampleFreq)))
                                        {
                                            memcpy(SampleFreq, USBHS_EP0_Buf, USBHS_SetupReqLen);
                                            CPLD_RequestFormat(freq);
                                            Audio_ResetFeedback(freq);
                                        }
                                    }
                                }
                            }
            ...
            case USBHS_UIS_TOKEN_OUT | DEF_UEP1:
                ...
                if (intst & USBHS_UIS_TOG_OK)
                {
                    /* Write In Buffer */
                    Audio_FillBuffer(
                        (uint8_t *)&USBHS_EP1_Rx_Buf[USBHS_EP1_Rx_Buf_PTR],
                        len);
                    USBHS_EP1_Rx_Buf_PTR = 1 - USBHS_EP1_Rx_Buf_PTR;
                    USBHSD->UEP1_RX_DMA =
                        (uint32_t)&USBHS_EP1_Rx_Buf[USBHS_EP1_Rx_Buf_PTR];
                    //                            write_log(USBHSD->FRAME_NO >> 13);
                    //                            write_log(len);
        ...
        case USBHS_UIS_TOKEN_SOF:
            freq = Audio_GetFeedback();
            USBHS_EP1_Tx_Buf[0] = freq;
            USBHS_EP1_Tx_Buf[1] = freq >> 8;
            USBHS_EP1_Tx_Buf[2] = freq >> 16;
            USBHS_EP1_Tx_Buf[3] = freq >> 24;
            USBHSD->UEP1_TX_LEN = 4;
            Audio_Sync(USBHSD->FRAME_NO >> 13);
            break;
    ...
    else if (intflag & USBHS_UIF_SETUP_ACT)
    {
        ...
        if ((USBHS_SetupReqType & USB_REQ_TYP_MASK) == USB_REQ_TYP_STANDARD)
        {
            ...
            case USB_GET_INTERFACE:
                USBHS_EP0_Buf[0] = USBHS_DevCurInterface;
                if (USBHS_SetupReqLen > 1)
                {
                    USBHS_SetupReqLen = 1;
                }
            ...
            case USB_SET_INTERFACE:
                USBHS_DevCurInterface = USBHS_SetupReqValue;
                if (USBHS_SetupReqIndex == 0x01 && USBHS_SetupReqValue == 0x00)
                {
                    // Audio Streaming Zero Bandwidth
                    Audio_SetUnmute(Bit_RESET);
                }
                else if (USBHS_SetupReqIndex == 0x01 && USBHS_SetupReqValue == 0x01)
                {
                    // Audio Streaming Operational 32Bits
                    Audio_SetBits(32);
                    Audio_SetUnmute(Bit_SET);
                }
                else if (USBHS_SetupReqIndex == 0x01 && USBHS_SetupReqValue == 0x02)
                {
                    // Audio Streaming Operational 16Bits
                    Audio_SetBits(16);
                    Audio_SetUnmute(Bit_SET);
                }
                else if (USBHS_SetupReqIndex == 0x01 && USBHS_SetupReqValue == 0x03)
                {
                    // Audio Streaming Operational 24Bits
                    Audio_SetBits(24);
                    Audio_SetUnmute(Bit_SET);
                }
                break;
        ...
        {
            if (USBHS_SetupReqType & AUDIO_REQ_GET_MSK) // GET
            {
                if (USBHS_SetupReqCode == AUDIO_REQ_CUR) // CUR
                {
                    if (USBHS_SetupReqIndex == (AUDIO_CLOCK_ID << 8) && USBHS_SetupReqValue == (CS_SAM_FREQ_CONTROL << 8)) // Clock
                    {
                        len = sizeof(SampleFreq);
                        pUSBHS_Descr = SampleFreq;
                    }
                    else
                    {
                        errflag = 0xFF;
                ...
                else if (USBHS_SetupReqCode == AUDIO_REQ_RANGE) // RANGE
                {
                    if (USBHS_SetupReqIndex == (AUDIO_CLOCK_ID << 8) && USBHS_SetupReqValue == (CS_SAM_FREQ_CONTROL << 8)) // Clock
                    {
                        len = sizeof(SampleRangeFreq);
                        pUSBHS_Descr = SampleRangeFreq;
                    }
                    else
                    {
                        errflag = 0xFF;
