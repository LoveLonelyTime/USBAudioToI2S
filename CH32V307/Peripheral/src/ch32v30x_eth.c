...
uint32_t ETH_HandleTxPkt(uint8_t *ppkt, uint16_t FrameLength) {
    ...
    for (offset = 0; offset < FrameLength; offset++) {
        (*(__IO uint8_t *) ((DMATxDescToSet->Buffer1Addr) + offset)) = (*(ppkt
                + offset));
    }
    ...
    if ((DMATxDescToSet->Status & ETH_DMATxDesc_TCH) != (uint32_t) RESET) {
        ...
        if ((DMATxDescToSet->Status & ETH_DMATxDesc_TER) != (uint32_t) RESET) {
            ...
            DMATxDescToSet = (ETH_DMADESCTypeDef *) ((uint32_t) DMATxDescToSet
...
uint32_t ETH_HandleRxPkt(uint8_t *ppkt) {
    ...
    if ((DMARxDescToGet->ControlBufferSize & ETH_DMARxDesc_RCH)
        ...
        if ((DMARxDescToGet->ControlBufferSize & ETH_DMARxDesc_RER)
            ...
            DMARxDescToGet = (ETH_DMADESCTypeDef *) ((uint32_t) DMARxDescToGet
                    + 0x10 + ((ETH->DMABMR & ETH_DMABMR_DSL) >> 2));
        }
    }
...
void ETH_DMATxDescRingInit(ETH_DMADESCTypeDef *DMATxDescTab, uint8_t *TxBuff1,
    ...
    for (i = 0; i < TxBuffCount; i++) {
        ...
        DMATxDesc->Buffer2NextDescAddr = (uint32_t) (&TxBuff2[i
                * ETH_MAX_PACKET_SIZE]);
...
void ETH_DMATxDescBufferSizeConfig(ETH_DMADESCTypeDef *DMATxDesc,
    ...
    DMATxDesc->ControlBufferSize |= (BufferSize1
            | (BufferSize2 << ETH_DMATXDESC_BUFFER2_SIZESHIFT));
}
...
void ETH_DMARxDescRingInit(ETH_DMADESCTypeDef *DMARxDescTab, uint8_t *RxBuff1,
    ...
    for (i = 0; i < RxBuffCount; i++) {
        ...
        DMARxDesc->Buffer2NextDescAddr = (uint32_t) (&RxBuff2[i
                * ETH_MAX_PACKET_SIZE]);
...
void ETH_DMAPTPTxDescChainInit(ETH_DMADESCTypeDef *DMATxDescTab,
        ETH_DMADESCTypeDef *DMAPTPTxDescTab, uint8_t *TxBuff,
        uint32_t TxBuffCount) {
    uint32_t i = 0;
    ETH_DMADESCTypeDef *DMATxDesc;
    ...
    for (i = 0; i < TxBuffCount; i++) {
        ...
        (&DMAPTPTxDescTab[i])->Buffer2NextDescAddr =
...
void ETH_DMAPTPRxDescChainInit(ETH_DMADESCTypeDef *DMARxDescTab,
        ETH_DMADESCTypeDef *DMAPTPRxDescTab, uint8_t *RxBuff,
        uint32_t RxBuffCount) {
    uint32_t i = 0;
    ETH_DMADESCTypeDef *DMARxDesc;
    ...
    for (i = 0; i < RxBuffCount; i++) {
        ...
        (&DMAPTPRxDescTab[i])->Buffer2NextDescAddr =
    ...
    *PTPTxTab = DMATxDescToSet->Buffer2NextDescAddr;
    ...
    if ((DMATxDescToSet->Status & ETH_DMATxDesc_TCH) != (uint32_t) RESET) {
        ...
        if (DMAPTPTxDescToSet->Status != 0) {
            DMAPTPTxDescToSet =
                    (ETH_DMADESCTypeDef *) (DMAPTPTxDescToSet->Status);
        } else {
            DMAPTPTxDescToSet++;
        }
    } else {
...
uint32_t ETH_HandlePTPRxPkt(uint8_t *ppkt, uint32_t *PTPRxTab) {
    ...
    if (((DMARxDescToGet->Status & ETH_DMARxDesc_ES) == (uint32_t) RESET)
        ...
        for (offset = 0; offset < framelength; offset++) {
            (*(ppkt + offset)) =
                    (*(__IO uint8_t *) ((DMAPTPRxDescToGet->Buffer1Addr)
                            + offset));
        }
    } else {
        framelength = ETH_ERROR;
    ...
    *PTPRxTab = DMARxDescToGet->Buffer2NextDescAddr;
    ...
    if ((DMARxDescToGet->ControlBufferSize & ETH_DMARxDesc_RCH)
        ...
        if (DMAPTPRxDescToGet->Status != 0) {
            DMAPTPRxDescToGet =
                    (ETH_DMADESCTypeDef *) (DMAPTPRxDescToGet->Status);
        } else {
            DMAPTPRxDescToGet++;
        }
    } else {
